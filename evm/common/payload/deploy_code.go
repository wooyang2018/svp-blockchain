...
func (dc *DeployCode) GetWasmCode() ([]byte, error) {
	if dc.VmType() == WASMVM_TYPE {
		...
		return nil, errors.New("not wasm contract")
	}
}
...
func (dc *DeployCode) GetNeoCode() ([]byte, error) {
	if dc.VmType() == NEOVM_TYPE {
		...
		return nil, errors.New("not neo contract")
	}
}
...
func validateDeployCode(dep *DeployCode) error {
	...
	if dep.VmType() == WASMVM_TYPE {
		if len(dep.code) > maxWasmCodeSize {
			return errors.New("[contract] Code too long!")
		}
	} else {
	...
	if len(dep.Name) > 252 {
		return errors.New("[contract] name too long!")
	}

	if len(dep.Version) > 252 {
		return errors.New("[contract] version too long!")
	}
	...
	if len(dep.Email) > 252 {
		return errors.New("[contract] email too long!")
	}

	if len(dep.Description) > 65536 {
		return errors.New("[contract] description too long!")
	}
