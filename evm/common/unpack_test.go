...
func TestUnpack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i)+" "+test.def, func(t *testing.T) {
			...
			out, err := a.Unpack("method", encb)
			if err != nil {
				t.Errorf("test %d (%v) failed: %v", i, test.def, err)
				return
			}
			if !reflect.DeepEqual(test.unpacked, abi.ConvertType(out[0], test.unpacked)) {
...
func TestUnpackIntoInterfaceSetDynamicArrayOutput(t *testing.T) {
	abi, err := abi.JSON(strings.NewReader(`[{"constant":true,"inputs":[],"name":"testDynamicFixedBytes15","outputs":[{"name":"","type":"bytes15[]"}],"payable":false,"stateMutability":"view","type":"function"},{"constant":true,"inputs":[],"name":"testDynamicFixedBytes32","outputs":[{"name":"","type":"bytes32[]"}],"payable":false,"stateMutability":"view","type":"function"}]`))
	if err != nil {
		t.Fatal(err)
	}
...
func methodMultiReturn(require *require.Assertions) (abi.ABI, []byte, methodMultiOutput) {
	const definition = `[
...
func TestUnpackTuple(t *testing.T) {
	...
	a, err := abi.JSON(strings.NewReader(simpleTuple))
	if err != nil {
		t.Fatal(err)
	}
	buff := new(bytes.Buffer)
	...
	var ret0 = new(r)
	err = a.UnpackIntoInterface(ret0, "tuple", buff.Bytes())
	...
	a, err = abi.JSON(strings.NewReader(nestedTuple))
	if err != nil {
		t.Fatal(err)
	}
	buff.Reset()
	...
	err = a.UnpackIntoInterface(&ret, "tuple", buff.Bytes())
	if err != nil {
		t.Error(err)
	}
